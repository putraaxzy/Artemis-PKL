...
export function isSessionExpired(error: any): boolean {
  ...
  if (error?.pesan?.includes('kadaluarsa') || error?.pesan?.includes('expired')) return true;
  return false;
}
...
export function isSessionInvalid(error: any): boolean {
  ...
  if (error?.pesan?.includes('tidak valid') || error?.pesan?.includes('tidak ditemukan')) return true;
  return false;
}
...
export function getSessionErrorState(error: any): sessionErrorState {
  return {
    ...
    message: error?.message || error?.pesan || 'terjadi kesalahan. silakan coba lagi.',
  };
}
...
export function getUserFriendlyErrorMessage(error: any): string {
  ...
  return error?.pesan || error?.message || 'terjadi kesalahan. silakan coba lagi.';
}
